...
type AttemptHandler interface {
	...
	PatchAttempt(c fuego.ContextWithBody[attempt.Attempt], id uuid.UUID) (*attempt.Attempt, error)
...
func (h *attemptHandler) PatchAttempt(c fuego.ContextWithBody[attempt.Attempt], id uuid.UUID) (*attempt.Attempt, error) {
	id, err := uuid.Parse(c.PathParam("id"))
	if err != nil {
		return nil, err
	}
	...
	attempt, err := h.repo.PatchAttempt(c.Context(), &body)
	if err != nil {
		return nil, fuego.InternalServerError{}
	}
